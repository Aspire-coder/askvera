...
def test_total_retrieval_unavailability_uses_dependency_copy_before_evidence_gate() -> None:
    """A provider outage must not claim that the approved documents are empty."""
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    body = ChatRequest(message="What are manager qualifications?", sessionId="session-1", country="US", language="en")
    ...
    response, returned, decision = orchestrator._route_or_approve_evidence(
        body.message,
        unavailable,
        body.message,
        body,
    ...
    assert response is not None
    assert returned is unavailable
    assert decision is None
    assert response.metadata["failure_layer"] == "dependency_unavailable"
    assert response.answer == (
        chat_orchestrator.localized_conversation_response("bedrock_error", "en")
        or chat_orchestrator.FALLBACK_RESPONSES["bedrock_error"]
    )


def test_evidence_routing_keeps_degraded_provider_availability(monkeypatch) -> None:
    """Partial OpenSearch failure stays visible after evidence approval."""
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    body = ChatRequest(message="What are manager qualifications?", sessionId="session-1", country="US", language="en")
    document = RetrievedDocument(
        id="us-policy",
        title="US policy",
        content="Manager qualifications are stated in section 5.",
        source="s3://approved/us-policy.pdf",
        country="US",
        language="en",
        score=0.9,
    ...
    assert approved.availability is RetrievalAvailability.DEGRADED


def test_degraded_retrieval_without_approved_evidence_uses_dependency_copy() -> None:
    """A partial provider outage without usable evidence is not a document miss."""
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    body = ChatRequest(message="What are manager qualifications?", sessionId="session-1", country="US", language="en")
    degraded = RetrievalResult(
        documents=[],
        citations=[],
        confidence=0.0,
        metadata={"failed_search_channels": ["vector"]},
        availability=RetrievalAvailability.DEGRADED,
    )
    ...
    assert response is not None
    assert returned is degraded
    assert decision is None
    assert response.metadata["failure_layer"] == "dependency_unavailable"
    assert response.metadata["retrieval_availability"] == "degraded"


def test_degraded_global_directory_loss_after_scope_reapproval_uses_dependency_copy(monkeypatch) -> None:
    """Foreign directory loss must not become a local-document evidence miss."""
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    body = ChatRequest(message="What is the Mexico office phone?", sessionId="session-1", country="US", language="en")
    local = RetrievedDocument(
        id="us-policy",
        title="US policy",
        content="US policy contact information.",
        source="s3://approved/us-policy.pdf",
        country="US",
        language="en",
        score=0.9,
    ...
    response, returned, decision = orchestrator._route_or_approve_evidence(
        ...
        body.message,
        body,
        "degraded-global-loss-cid",
    )

    assert response is not None
    assert returned.availability is RetrievalAvailability.DEGRADED
    assert decision is None
    assert response.metadata["failure_layer"] == "dependency_unavailable"
    assert response.metadata["retrieval_availability"] == "degraded"


def test_degraded_global_evidence_routes_normally(monkeypatch) -> None:
    """A global directory answer can still use valid evidence from a partial request."""
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    body = ChatRequest(message="What is the Mexico office phone?", sessionId="session-1", country="US", language="en")
    document = RetrievedDocument(
        id="mexico-office",
        title="International office directory - Mexico",
        content="Mexico office phone details.",
        source="s3://approved/global-directory.pdf",
        country="GLOBAL",
        language="en",
        score=0.9,
...
def test_degraded_cross_market_policy_refusal_remains_an_evidence_gate(monkeypatch) -> None:
    """A foreign policy request is a scope refusal, not a dependency outage."""
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    body = ChatRequest(
        message="What does Mexico company policy say about manager qualifications?",
        sessionId="session-1",
        country="US",
        language="en",
    )
    local = RetrievedDocument(
        id="us-policy",
        title="US policy",
        content="US manager qualifications.",
        source="s3://approved/us-policy.pdf",
        country="US",
        language="en",
        score=0.9,
    ...
    response, returned, decision = orchestrator._route_or_approve_evidence(
        ...
        body.message,
        body,
        "degraded-cross-market-policy-cid",
    )
...
def test_evidence_contract_reconstruction_preserves_provider_availability(monkeypatch, availability) -> None:
    """Contract acceptance must preserve availability for every provider state."""
    monkeypatch.setattr(settings, "EVIDENCE_GATED_OUTPUT_ENABLED", True)
    orchestrator = AIOrchestrator(validator=_FakeValidator(), governance=_FakeGovernance())
    document = RetrievedDocument(
        id="section-1",
        title="US policy section 1",
    ...
    contracted = orchestrator._apply_evidence_contract(model_response, retrieval_result, "contract-cid")

    assert contracted is not None
    assert contracted[1].availability is availability
...
def test_country_demonym_question_is_not_sent_to_typo_confirmation(monkeypatch) -> None:
    retriever = MagicMock()
    retriever.retrieve.return_value = RetrievalResult(documents=[], citations=[], confidence=0.0)
    router = MagicMock()
    router.generate.side_effect = chat_orchestrator.RetrievalMissError("no evidence")
    ...
    body = ChatRequest(
        message="What is today's exchange rate from euros to Tunisian dinars?",
        sessionId="session-1", country="US", language="en",
    )
    monkeypatch.setattr(chat_orchestrator, "validate_and_touch_session", lambda *_: None)
    monkeypatch.setattr(chat_orchestrator, "has_valid_consent", lambda *_: True)
    monkeypatch.setattr(chat_orchestrator, "scrub_pii", lambda text, *_, **__: text)
    ...
    assert response.metadata.get("response_source") != "market_typo_confirmation"
    retriever.retrieve.assert_called_once()
