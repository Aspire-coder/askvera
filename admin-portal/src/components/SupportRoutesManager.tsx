...
export function SupportRoutesManager({ credentials, config }: { credentials: AdminCredentials; config: AdminConfig }) {
  ...
  return <section className="page-section">
    ...
    <div className="user-toolbar surface"><label className="toolbar-field"><span>Market</span><select aria-label="Select support market" value={selectedRoute?.country || ""} onChange={(event) => setSelectedCountry(event.target.value)}>{rows.map((route) => <option key={route.country} value={route.country}>{route.country_name}</option>)}</select></label><span className="mode-note">Choose a market to edit its handoff route.</span></div>
