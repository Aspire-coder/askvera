...
| D | **partly integrated** (`deb3a8c`); round 2 active | Shared span-aware splitter `utils/sentence_spans.py`, used by evidence_contract and the response builder; audit of every editor in FRAGMENT_AUDIT.md; "after sponsorship" decimal cut fixed | coordinator: exit 0 on the affected suites | The coordinator rejected part of the directory patch as delivered (it would have swallowed the next field line) and fixed it, with a test. The validator patch for orphaned abbreviations deleted a correct neighbouring sentence on the combined candidate and broke an approved test, so it was reverted; 3 strict xfails remain while round 2 works on it |
| E | active | 0 | - | - |
...
Phase 2 complete: 57% (4 of 7 lanes fully integrated (B, C, F, G), D partly; A and E outstanding; Fable review pending).
