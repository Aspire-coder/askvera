...
|---|---|---|---|---|
| A | **integrated** (`53d8f01` + round 2 `3b3960b`) | A7 fixed: "the other one" with 2 or more session markets gives one clarification naming them; "the first one" resolves by first mention; built from user turns only, no model call; closed-class vocabulary for ten languages; hook in `_handle_chat`, away from Codex's regions. Coordinator probe: en, fr, de and es positives clarify; first order, last day, first requirement, other fee, other product, and first order in fr and de are all unchanged; the unsafe-plus-reference message takes the normal path and is refused | lane: 679 passed, exit 0; coordinator: exit 0 on the follow-up, reference, compound and tone suites; A7 strict xfail flipped to pass | Round 1 appended "Kenya" to "minimum first order" and bypassed governance for an unsafe message; round 2 counts a reference only when nothing else remains except a prop-word. "And the other country?" under-resolves (left unchanged, the safe direction). pt has no routes locale, so it falls back to translation. The clarification can show ISO long names (Codex-owned market_config). A pure-reference message is answered before the sensitive-PII notice; it cannot carry PII, because any leftover content disqualifies it |
...
| C | **integrated** (`c4c7d52`) | A timing figure is grounded only under a compatible process stage (delivery, approval, processing, payment, settlement, waiting period, office hours) in 11 language entries; a mismatch becomes NUMERIC_CLAIM_UNGROUNDED and the existing repair removes only that sentence; no new editor. Coordinator probes: delivery to approval, waiting period to processing, and payment date to bank settlement (both "arrives in your account" and "credited") are all flagged; same-stage rewordings pass | lane: 804 passed, exit 0; coordinator: exit 0, flake8 clean | Round 1 missed the brief's own example (topic nouns in the stage cues); fixed in round 2. Processing time deliberately does not ground an approval time (trade-off, pinned). A source sentence naming two stages is unclassified and grounds any stage (conservative). Cues are exact words, not stems, so an unlisted conjugation is simply unclassified. Nordic terms are translated, not sourced. The proposed prompt rule was declined: over budget, and redundant with the existing "timeframes are source-locked" rule |
| D | **integrated** (`deb3a8c` + round 2 `56b3bf3`) | Shared span-aware splitter `utils/sentence_spans.py` (evidence_contract, response builder); FRAGMENT_AUDIT.md covers every editor; two confirmed fragment defects fixed: the "after sponsorship" correction cut at a decimal, and numeric repair orphaned "approx.", "Nr." and initials | coordinator: exit 0, flake8 clean; the approved bracket test still passes | Round 1's directory patch would have swallowed the next field line (coordinator fix and test). Round 1's validator patch deleted a correct sentence on the combined candidate: its splitter required uppercase after a stop and "(" is not uppercase. Round 2 leaves the boundary regex alone and only subtracts abbreviation and initial periods. Label-anchored editors were spot-probed, not fuzzed |
| E | active | 0 | - | - |
...
| G | **integrated** (`f69be01`) | Default collection now covers tests/unit, tests/governance, tests/conversation and tests/conversation_pack (172 to 185 files; integration stays opt-in), guarded by a collection-integrity test; A1 rewritten so the fake model echoes the stale number and the REAL validator must remove it (asserts `numeric_claim_repair`); a module-label check; COVERAGE.md | lane: 8915 passed, 5 skipped, 14 xfailed, exit 0 | Its label check caught three unlabelled modules from Lanes C and F after they merged; the coordinator labelled them and removed the one known-gap exemption, so no label skip remains |

Phase 2 complete: 86% (6 of 7 lanes integrated: A, B, C, D, F, G; E outstanding; Fable review pending).
...
- Lane B's orchestrator patch was malformed (bad hunk header); its three one-line edits were applied by hand.
- flake8 C901: `_handle_scrubbed_chat` reached complexity 16 (max 15) at Phase 1 `dbc6a7a`, which I missed because Phase 1 never ran flake8. Lane E's wiring must extract the dependency paths and bring it back to 15 or below.
- Codex conflict baseline: Codex's current orchestrator diff conflicts with this branch in 2 hunks, both at the Phase 1 retrieve() catch. Resolution is mechanical (Codex's token set before `try`, our `except`, Codex's `finally`).
