...
| `app/validation/validators/numeric_grounding_validator.py : remove_unsupported_numeric_sentences` | Bare `[.!?](?=\s\|$)\|\n` boundary search, guarded only against a 2+-repeat letter-dot run (`e.g.`, `U.S.`) | decimals (already fixed), single abbreviations (`approx.`, `ca.`, `Nr.`), initials (`J. R.`), emails, URLs, phone numbers, currencies, clause references | **defect confirmed**: a single abbreviation or an initial before a deleted unsupported number leaves it orphaned - `"The fee is approx. Payment methods accepted: cash."`, `"See policy Nr. Payment methods accepted: cash."`, `"Contact J. R. Payment methods accepted: cash."`. `xfail` at `tests/conversation/test_fragment_numeric_repair_abbreviations.py`; patch at `patches/laneD-numeric-grounding-validator.patch`. **Lane C file, not edited on this branch** |
