...
| 7 | Admin MFA | Reverted to optional (2026-09) | Cognito MFA (`MfaConfiguration`) was mandatory in the operations portal infrastructure template, then reverted to `OPTIONAL` after it blocked new-administrator sign-in - Cognito's hosted UI forces authenticator-app enrollment before returning control to the app. TOTP remains available to any administrator who opts in. |
